...
qas::State deterministic_position(int plies, int salt) {
    ...
    for (int ply = 0; ply < plies && state.terminal == qas::Terminal::None; ++ply) {
        ...
        qas::Undo undo;
        if (!qas::apply_move(state, moves[index], undo))
            std::abort();
    }
    if (state.terminal != qas::Terminal::None)
        return qas::initial_state();
    return state;
}
...
int main(int argc, char** argv) {
    ...
    std::cout << "mode,position,depth,score,legal,nodes,expanded,cutoffs,avg_L,tt_hits,tt_probes,"
                 "time_ms,best\n";
